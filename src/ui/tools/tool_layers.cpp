...
void DrawPanelLayers(UIContext *ctx, AppConfig *cfg)
{
    ...
    bool enabled=ToolSettingGetBool(cfg,BORDERS_KEY,false); if(ImGui::Checkbox("2D Country Borders",&enabled)) ToolSettingSetBool(cfg,BORDERS_KEY,enabled);
...
void DrawSceneLayers(SceneContext *sctx,AppConfig *cfg){if(!sctx->is_2d_view||!sctx->camera2d||!ToolSettingGetBool(cfg,BORDERS_KEY,false))return; Color c=g_theme.ui.text_main; c.a=(unsigned char)(c.a*0.20f); float width=0.8f/fmaxf(sctx->camera2d->zoom,0.10f); for(int i=0;i<MAP_BORDER_LINE_COUNT;++i){int start=MAP_BORDER_LINES[i].start,count=MAP_BORDER_LINES[i].count;for(int j=1;j<count;++j){Vector2 a=to_world(MAP_BORDER_POINTS[start+j-1],sctx->map_w,sctx->map_h),b=to_world(MAP_BORDER_POINTS[start+j],sctx->map_w,sctx->map_h);if(fabsf(a.x-b.x)<=sctx->map_w*0.45f)DrawLineEx(a,b,width,c);}}}
